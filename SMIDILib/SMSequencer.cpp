...
//******************************************************************************
//現在の演奏Tick位置を取得
//  m_TotalTickTimeTemp はタイマースレッドで毎回更新される最新の演奏Tick。
//  32bit値の読み取りはアトミックなので別スレッドから参照しても問題ない。
//  メッセージキューに頼らず位置を取得できるため高負荷時にも確実。
//******************************************************************************
...
//******************************************************************************
//現在の演奏時間(msec)を取得
//  m_CurPlayTime はナノ秒単位。1000000で割るとmsec。
//******************************************************************************
...
//******************************************************************************
// 演奏位置を曲の先頭に戻す（停止中のみ）
//   再生終了後にビューを末尾へ残す DX9 と異なり、停止ボタン／曲の読み込みでは
//   先頭へ戻したい。Play と同じ初期化ルーチンで位置状態だけ先頭にリセットする。
//******************************************************************************
int SMSequencer::Rewind()
{
	int result = 0;
	//演奏中・一時停止中は不可（タイマースレッドが位置を更新しているため）
	if (m_Status != StatusStop) goto EXIT;
	if (m_pSeqData == NULL) goto EXIT;
	//Play 開始時と同じ位置初期化（GetCurrentTickTime/PlayTime が 0 を返すようになる）
	result = _InitializeParamsOnPlayStart();
	if (result != 0) goto EXIT;
EXIT:;
	return result;
