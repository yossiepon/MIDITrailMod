	...
	)
{
	...
	for (i = 0; i < chunkDataSection.ntracks; i++) {
		...
		//32bit上限に達したら以降のトラックは取り込めない（ファイル位置もトラック途中で
		//止まっている）。読み込めた分で確定し、残りのトラックは読まずに抜ける。
	...
	)
{
	...
	while (readSize < chunkSize) {
		...
		//32bit上限に達したら以降のイベントは取り込めない。これ以上読んでも捨てるだけ
		//なので、ここで打ち切って読み込めた分を返す（破壊せず・フラグは AddItem が設定）。
		if (SMSimpleList::WasTruncated()) break;
