...
int SMLiveMonitor::_OpenMIDIDev()
{
	...
	//出力ポートのデバイスを開く
	// MIDITHRU (passing input to a synth) is optional: if the OUT device can't be
	// opened, disable THRU and keep monitoring + visualization running instead of
	// aborting the whole monitor start.
	if (strlen(m_OutPortDevName) > 0) {
		if (m_OutDevCtrl.SetPortDev(0, m_OutPortDevName) != 0) {
			m_OutPortDevName[0] = '\0';   // THRU unavailable; continue without it
		}
	}
