...
float MTNoteDesignMod::GetRippleOffsetY()
{
	return m_RippleOffsetY;
}

float MTNoteDesignMod::GetRippleOffsetZ()
{
	return m_RippleOffsetZ;
}
...
void MTNoteDesignMod::_Clear(void)
{
	...
	m_RippleOffsetY = 0.0f;
	m_RippleOffsetZ = 0.0f;
}
	...
	)
{
	...
	//ripple position offset (default 0.0 = note center), independent of size.
	//OffsetY shifts along pitch (Y); OffsetZ shifts along depth/channel (Z).
	result = confFile.GetFloat(_T("OffsetY"), &m_RippleOffsetY, 0.0f);
	if (result != 0) goto EXIT;
	result = confFile.GetFloat(_T("OffsetZ"), &m_RippleOffsetZ, 0.0f);
	if (result != 0) goto EXIT;
