...
//******************************************************************************
// M3 DX11: device-free init (note design + bar/port counts)
//******************************************************************************
int MTGridBox::InitForDX11(const TCHAR* pSceneName, SMSeqData* pSeqData)
{
	int result = 0;
	SMBarList barList;
...
//******************************************************************************
// M3 DX11: vertex / index counts (call after InitForDX11)
//******************************************************************************
void MTGridBox::GetGeometrySize(unsigned long* pVertexNum, unsigned long* pIndexNum)
{
	unsigned long ports = m_PortList.GetSize();
	if (ports < 1) ports = 1;
	if (pVertexNum != NULL) *pVertexNum = 8 + (2 * m_BarNum) + (4 * (ports - 1));
	if (pIndexNum != NULL)  *pIndexNum  = 24 + (2 * m_BarNum) + (4 * (ports - 1));
}

//******************************************************************************
// M3 DX11: fill caller CPU buffers with grid geometry (device-free).
//   Vertex layout is XYZ|NORMAL|DIFFUSE (28 B) -- caller converts to DXP11_VERTEX.
//******************************************************************************
int MTGridBox::BuildGeometryCPU(SMSeqData* pSeqData, void* pVertexBuf, unsigned long* pIndexBuf)
{
	int result = 0;
	SMBarList barList;
	unsigned long totalTickTime = 0;
	MTGRIDBOX_VERTEX* pVertex = (MTGRIDBOX_VERTEX*)pVertexBuf;

	result = pSeqData->GetBarList(&barList);
	if (result != 0) goto EXIT;
	totalTickTime = pSeqData->GetTotalTickTime();

	result = _CreateVertexOfGrid(pVertex, pIndexBuf, totalTickTime);
	if (result != 0) goto EXIT;
	result = _CreateVertexOfBar(&(pVertex[8]), &(pIndexBuf[24]), 8, &barList);
	if (result != 0) goto EXIT;
	result = _CreateVertexOfPortSplitLine(
					&(pVertex[8 + (2 * m_BarNum)]),
					&(pIndexBuf[24 + (2 * m_BarNum)]),
					8 + (2 * m_BarNum),
					totalTickTime);
	if (result != 0) goto EXIT;
...
//******************************************************************************
D3DXVECTOR3 MTGridBox::GetWorldMoveVector()
{
	return m_NoteDesign.GetWorldMoveVector();
}
