	...
	)
{
	...
	if (m_KeyboardDesign.GetKeyDispRangeEnd() == noteNo && !m_KeyboardDesign.IsInfiniteKeyboard()) {
	...
	)
{
	...
	if (m_KeyboardDesign.GetKeyDispRangeStart() == noteNo && !m_KeyboardDesign.IsInfiniteKeyboard()) {
...
//******************************************************************************
//******************************************************************************
// M3 DX11: initialize the keyboard design only (device-free)
//******************************************************************************
int MTPianoKeyboard::InitForDX11(const TCHAR* pSceneName, SMSeqData* pSeqData)
{
	return m_KeyboardDesign.Initialize(pSceneName, pSeqData);
}

//******************************************************************************
// M3 DX11: total vertex / index counts (call after InitForDX11)
//******************************************************************************
void MTPianoKeyboard::GetGeometrySize(unsigned long* pVertexNum, unsigned long* pIndexNum)
{
	unsigned long vn = 0, in = 0;
	unsigned char noteNo;
	_CreateBufInfo();
	for (noteNo = 0; noteNo < SM_MAX_NOTE_NUM; noteNo++) {
		vn += m_BufInfo[noteNo].vertexNum;
		in += m_BufInfo[noteNo].indexNum;
	}
	if (pVertexNum != NULL) *pVertexNum = vn;
	if (pIndexNum != NULL)  *pIndexNum = in;
}

//******************************************************************************
// M3 DX11: fill caller CPU buffers with keyboard geometry (device-free)
//   Reuses the exact DX9 per-key builders; the vertex layout is identical to
//   DXP11_VERTEX so the buffer uploads straight to D3D11.
//******************************************************************************
int MTPianoKeyboard::BuildGeometryCPU(void* pVertexBuf, unsigned long* pIndexBuf)
{
	int result = 0;
	unsigned char noteNo;
	MTPIANOKEYBOARD_VERTEX* pAll = (MTPIANOKEYBOARD_VERTEX*)pVertexBuf;

	_CreateBufInfo();

	for (noteNo = 0; noteNo < SM_MAX_NOTE_NUM; noteNo++) {
		MTPIANOKEYBOARD_VERTEX* pv = &pAll[m_BufInfo[noteNo].vertexPos];
		unsigned long* pi = &pIndexBuf[m_BufInfo[noteNo].indexPos];
		ZeroMemory(pv, m_BufInfo[noteNo].vertexNum * sizeof(MTPIANOKEYBOARD_VERTEX));
		ZeroMemory(pi, m_BufInfo[noteNo].indexNum * sizeof(unsigned long));

		switch (m_KeyboardDesign.GetKeyType(noteNo)) {
			case MTPianoKeyboardDesign::KeyWhiteC:
			case MTPianoKeyboardDesign::KeyWhiteF:
				result = _CreateVertexOfKeyWhite1(noteNo, pv, pi); break;
			case MTPianoKeyboardDesign::KeyWhiteD:
			case MTPianoKeyboardDesign::KeyWhiteG:
			case MTPianoKeyboardDesign::KeyWhiteA:
				result = _CreateVertexOfKeyWhite2(noteNo, pv, pi); break;
			case MTPianoKeyboardDesign::KeyWhiteE:
			case MTPianoKeyboardDesign::KeyWhiteB:
				result = _CreateVertexOfKeyWhite3(noteNo, pv, pi); break;
			case MTPianoKeyboardDesign::KeyBlack:
				result = _CreateVertexOfKeyBlack(noteNo, pv, pi); break;
		}
		if (result != 0) goto EXIT;
	}

	// collapse keys outside the display range (same as _HideKey)
	for (noteNo = 0; noteNo < SM_MAX_NOTE_NUM; noteNo++) {
		if (!m_KeyboardDesign.IsKeyDisp(noteNo)) {
			MTPIANOKEYBOARD_VERTEX* pv = &pAll[m_BufInfo[noteNo].vertexPos];
			unsigned long i;
			for (i = 0; i < m_BufInfo[noteNo].vertexNum; i++) {
				pv[i].p = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
				pv[i].n = D3DXVECTOR3(0.0f, 0.0f, 1.0f);
				pv[i].c = 0;
				pv[i].t = D3DXVECTOR2(0.0f, 0.0f);
			}
		}
	}
...
//******************************************************************************
// M3 DX11: per-key vertex range
//******************************************************************************
void MTPianoKeyboard::GetKeyVertexRange(unsigned char noteNo, unsigned long* pVertexPos, unsigned long* pVertexNum)
{
...
//******************************************************************************
// M3 DX11: build one key (optionally pressed + tinted) into a CPU slice
//   Mirrors _RotateKey but writes to pDst instead of the DX9 primitive.
//******************************************************************************
int MTPianoKeyboard::BuildKeyCPU(unsigned char noteNo, float keyDownRate, D3DXCOLOR* pColor, void* pDst)
{
	int result = 0;
	unsigned long i = 0;
	MTPIANOKEYBOARD_VERTEX tempVertex[MTPIANOKEYBOARD_KEY_VERTEX_NUM_MAX];
	unsigned long tempIndex[MTPIANOKEYBOARD_KEY_INDEX_NUM_MAX];
	MTPIANOKEYBOARD_VERTEX* pOut = (MTPIANOKEYBOARD_VERTEX*)pDst;
	float angle = 0.0f;
	float centerZ = 0.0f;

	if (noteNo >= SM_MAX_NOTE_NUM) return YN_SET_ERR("Program error.", noteNo, 0);

	ZeroMemory(tempVertex, sizeof(tempVertex));
	ZeroMemory(tempIndex, sizeof(tempIndex));

	// keys outside the display range collapse to a point (invisible)
	if (!m_KeyboardDesign.IsKeyDisp(noteNo)) {
		for (i = 0; i < m_BufInfo[noteNo].vertexNum; i++) {
			pOut[i].p = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
			pOut[i].n = D3DXVECTOR3(0.0f, 0.0f, 1.0f);
			pOut[i].c = 0;
			pOut[i].t = D3DXVECTOR2(0.0f, 0.0f);
		}
		return 0;
	}
	...
	switch (m_KeyboardDesign.GetKeyType(noteNo)) {
		case MTPianoKeyboardDesign::KeyWhiteC:
		case MTPianoKeyboardDesign::KeyWhiteF:
			result = _CreateVertexOfKeyWhite1(noteNo, tempVertex, tempIndex, pColor); break;
		case MTPianoKeyboardDesign::KeyWhiteD:
		case MTPianoKeyboardDesign::KeyWhiteG:
		case MTPianoKeyboardDesign::KeyWhiteA:
			result = _CreateVertexOfKeyWhite2(noteNo, tempVertex, tempIndex, pColor); break;
		case MTPianoKeyboardDesign::KeyWhiteE:
		case MTPianoKeyboardDesign::KeyWhiteB:
			result = _CreateVertexOfKeyWhite3(noteNo, tempVertex, tempIndex, pColor); break;
		case MTPianoKeyboardDesign::KeyBlack:
			result = _CreateVertexOfKeyBlack(noteNo, tempVertex, tempIndex, pColor); break;
	}
	...
	if (keyDownRate > 0.0f) {
		...
		centerZ = m_KeyboardDesign.GetKeyRotateAxisXPos();
		for (i = 0; i < m_BufInfo[noteNo].vertexNum; i++) {
			tempVertex[i].p = DXH::RotateYZ(0.0f, centerZ, tempVertex[i].p, angle);
			tempVertex[i].n = DXH::RotateYZ(0.0f, 0.0f,    tempVertex[i].n, angle);
		}
	}

	memcpy(pOut, tempVertex, m_BufInfo[noteNo].vertexNum * sizeof(MTPIANOKEYBOARD_VERTEX));
	return 0;
}

//******************************************************************************
// M3 DX11: resolve the keyboard texture file path (device-free)
//******************************************************************************
int MTPianoKeyboard::GetTexturePath(const TCHAR* pSceneName, TCHAR* pOutPath, int maxLen)
{
	int result = 0;
	TCHAR bmpFileName[_MAX_PATH] = {_T('\0')};
	MTConfFile confFile;

	if ((pOutPath == NULL) || (maxLen <= 0)) return YN_SET_ERR("Program error.", 0, 0);
	pOutPath[0] = _T('\0');
	...
	result = YNPathUtil::GetModuleDirPath(pOutPath, maxLen);
	if (result != 0) goto EXIT;
	_tcscat_s(pOutPath, maxLen, bmpFileName);
