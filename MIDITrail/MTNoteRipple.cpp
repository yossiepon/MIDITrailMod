...
//******************************************************************************
// M3 DX11: device-free init (note design + status, no DX9 device)
//******************************************************************************
int MTNoteRipple::InitForDX11(const TCHAR* pSceneName, SMSeqData* pSeqData, MTNotePitchBend* pNotePitchBend)
{
	int result = 0;
	Release();
	result = _CreateNoteDesign();
...
//******************************************************************************
// M3 DX11: update ripple quads into a CPU buffer (device-free). 6 verts/ripple.
//******************************************************************************
int MTNoteRipple::UpdateVertexCPU(D3DXVECTOR3 camVector, void* pVertexBuf, unsigned long* pActiveNum)
{
	...
	if ((pVertex == NULL) || (m_pNoteStatus == NULL) || m_isSkipping || !m_isEnable) {
		m_ActiveNoteNum = 0;
	...
	for (i = 0; i < MTNOTERIPPLE_MAX_RIPPLE_NUM; i++) {
		if (m_pNoteStatus[i].isActive) {
			_SetVertexPosition(&(pVertex[activeNoteNum*6]), &(m_pNoteStatus[i]), activeNoteNum, curTime, &isTimeout);
			if (isTimeout) m_pNoteStatus[i].isActive = false;
			else activeNoteNum++;
		}
	}
	m_ActiveNoteNum = activeNoteNum;
...
//******************************************************************************
// M3 DX11: resolve the ripple texture path (device-free)
//******************************************************************************
int MTNoteRipple::GetTexturePath(const TCHAR* pSceneName, TCHAR* pOutPath, int maxLen)
{
	int result = 0;
...
//******************************************************************************
// M3 DX11: world move vector
//******************************************************************************
D3DXVECTOR3 MTNoteRipple::GetWorldMoveVector()
{
	if (m_pNoteDesign != NULL) return m_pNoteDesign->GetWorldMoveVector();
